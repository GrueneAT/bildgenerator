...
`+logoText.substring(lastSpace).trim())}textScaleTo=AppConstants.LOGO.TEXT_SCALE_LONG}else logoFilename=AppConstants.LOGO.FILES.SHORT,isLongLogo=!1,textScaleTo=AppConstants.LOGO.TEXT_SCALE_SHORT;scaleTo<121&&(logoFilename=logoFilename.replace("245","120").replace("248","121").replace("268","131")),fabric.Image.fromURL(generatorApplicationURL+"resources/images/logos/"+logoFilename,function(image){if(!(!image||!image.width||!image.height))try{image.scaleToWidth(scaleTo),image.lockMovementX=!0,image.lockMovementY=!0;const logoHeight=image.getScaledHeight(),template2=currentTemplate();image.top=calculateLogoTop(logoHeight,template2),CanvasUtils.disableScalingControls(image),image.selectable=!1,canvas.add(image),canvas.centerObjectH(image),canvas.bringToFront(image),logo=image;const offsetFromTop=image.getScaledWidth()*AppConstants.LOGO.PINK_BAR_OFFSET_FROM_TOP,textTopPosition=image.top+offsetFromTop;logoName=new fabric.Text(logoText,{top:textTopPosition,fontFamily:AppConstants.FONTS.DEFAULT_LOGO,fontSize:Math.floor(image.getScaledWidth()/10),fontStyle:"normal",textAlign:"right",fill:AppConstants.COLORS.LOGO_TEXT,stroke:AppConstants.COLORS.TEXT_STROKE,strokeWidth:0,objectCaching:!1,lineHeight:AppConstants.LOGO.LINE_HEIGHT,angle:AppConstants.LOGO.ANGLE,selectable:!1}),canvas.add(logoName);const linebreak=logoText.lastIndexOf(`
